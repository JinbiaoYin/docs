...
### `ConcurrentHashMap`是如何实现的？`Segment`是什么？
...
### `CAS`是什么？`CAS`的`ABA`问题
`CAS`是一种比较再交换算法。jdk5增加了并发包`java.util.concurrent.*`，其下面大量的类使用`CAS`算法区别`synchronized`同步锁的一种乐观锁。

`CAS`类似于乐观锁思想的一种实现，在不加锁的情况下，保证多线程下的数据一致性。

`CAS`导致的`ABA`问题，因为`CAS`每次暂存修改前的值，要修改时，再比较一次，如果相等再修改。但是在这个过程种，原值可能由`A`变为`B`再变为`A`了，`CAS`无法保证此时的`A`还是原来的`A`，这就是`CAS`导致的`ABA`问题。可以另外添加一个版本号`version`来解决`ABA`问题，每次修改值，版本号增加。

这样我想到一个问题，如果比较相等后未修改值前，其它线程对值进行了修改，不是不能保证线程数据的一致性了吗。但是`CAS`底层的实现是`public final native boolean compareAndSwapXXX`，它调用本地hotspot方法，本地方法中调用`cmpxchg`方法，在多个线程时，会进行`lock`操作。即`lock cmpxchg 指令`。所以`CAS`已经在底层支持了。

## 对象的内存布局
对象在内存中由4个部分组成
- 对象头（markword）。8个字节。
- 类型指针（jvm默认开启压缩指针，所有的引用类型都是4个字节）。4个字节。
- 实例数据。成员变量的大小，例如如果有个`int`成员变量就是4个字节，再有一个`long`成员变量就是`4+8`占12个字节。没有成员变量就是0个字节。
- 对齐。如果对象的大小不能被8整除，就补齐。

因此`Object  o = new Object()`在内存中，对象本身占了16个字节。如果算上引用`o`的4个字节共占20个字节。

### 无锁态，偏向锁，重量级锁
对象刚刚`new`出来是无锁态。对象被访问时，对象的`markword`中，前54位用来记录当前线程指针，此时升级为偏向锁。当有另一个线程来访问对象，就会产生锁竞争，此时先撤销偏向锁，两个线程通过`CAS`操作来将自己的`Lock Record`指针写入`markword`前62位。

当线程竞争比较激烈时，升级为重量级锁。向操作系统申请资源。

### `StringBuilder`和`StringBuffer`
`StringBuffer`线程安全，`StringBuilder`线程不安全。`StringBuffer`所有的公开方法都是被`synchronized`修饰的，而`StringBuilder`并没有`synchronized`修饰。

### `volatile`
当成员变量被`volatile`修饰，保证了这个变量的线程可见性。

`volatile`做到了缓存行（cache line）的一致性。