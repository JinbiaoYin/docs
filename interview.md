...
### `volatile`的作用和实现
#### 作用
当成员变量被`volatile`修饰，保证了这个变量的线程可见性，禁止指令重排序。

#### 实现
CPU存在三层缓存架构，`volatile`使缓存行对齐，`volatile`做到了缓存行（cache line）的一致性。

CPU存在乱序执行的问题。

当变量被`volatile`修饰，jvm会在该变量所在的内存区域的前后加上一层屏障，叫内存屏障。只有量`volatile`内存被读或者写后，才会继续进行下面的操作，就是为了防止指令重排序。
